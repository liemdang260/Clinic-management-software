...
class Stack {
  ...
  changeStatus(order, status) {
    ...
    for (let i = 0; i < this.patientStack.length; i++) {
      if (
        this.patientStack[i] != null &&
        this.patientStack[i].status != "appointment" &&
        this.patientStack[i].status != "diagnosed"
      ) {
  ...
  async initAppointment() {
    ...
    appointmentToday.map((appointment) => {
      ...
      this.patientStack[(date.getTime() - today()) / 900000] = {
        appointment,
        status: "appointment",
      };
    });
